...
ffmpeg:
  ...
  retry_interval: 10
  # Optional: Time in seconds without a decoded frame before the camera watchdog kills and restarts ffmpeg. (default: shown below)
  # The default of 20 is appropriate for most RTSP/RTMP sources. Increase for sources with bursty keyframe cadence
  # (for example WebRTC-bridged streams that pause briefly during renegotiation) to avoid spurious restarts.
  no_frames_threshold: 20
