...
- **Device**: Target a specific device to run the face recognition model on. This setting is only applicable when using the `large` model.
  ...
  - Set to `hailo` to run a precompiled ArcFace MobileFaceNet model on a Hailo-8 / Hailo-8L NPU. The HEF is downloaded on first use from the Hailo Model Zoo and shares the chip with the existing Hailo object detector via the HailoRT scheduler. Same 99.4% LFW accuracy as the ONNX model at INT8.
  - Default: `None`
